# 🚀 Hi there, I'm Alexis Albarenga

<p align="center">
  <img src="./assets/Banner.gif" width="100%" alt="Banner">
</p>

### 👨‍💻 Full-Stack Developer | Multiplatform Software Technician
I am a Software Developer graduated from the **Instituto Politécnico Formosa (IPF)**. I specialize in building robust multiplatform solutions, with a deep passion for **Backend architecture** and **Artificial Intelligence**.

---

### 🤖 AI & Backend Focus
Currently, I'm diving deep into the AI ecosystem, building agents and automated workflows.
- 🛠️ **Building with:** LangChain, Ollama, and n8n.
- ⚙️ **Backend Expert:** Crafting scalable APIs with NestJS and FastAPI.
- ☁️ **Deployment:** Dockerized environments and Cloudflare infrastructure.

---

### 🛠️ Tech Stack & Tools

**Languages & Core**

[![My Skills](https://skillicons.dev/icons?i=ts,js,py,php,html,css)](https://skillicons.dev)

**Backend & Databases**

[![Backend](https://skillicons.dev/icons?i=nestjs,fastapi,nodejs,postgres,mysql,mongodb,prisma,docker)](https://skillicons.dev)

**Frontend & UI**

[![Frontend](https://skillicons.dev/icons?i=react,tailwind,framer)](https://skillicons.dev)

**Tools & Automation**
<p align="left">
  <img src="https://img.shields.io/badge/Ollama-000000?style=flat-square&logo=ollama&logoColor=white" />
  <img src="https://img.shields.io/badge/n8n-FF6D5B?style=flat-square&logo=n8n&logoColor=white" />
  <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white" />
  <img src="https://img.shields.io/badge/Cloudflare-F38020?style=flat-square&logo=cloudflare&logoColor=white" />
</p>

---

### 🌎 Language
| Tool | Proficiency |
| :--- | :--- |
| **Spanish** | Native 🇦🇷 |
| **English** | Basic (A2) 🇺🇸 |

---

### 📨 Let's Connect!
- 🌐 **Portfolio:** [alexisalbarenga.pages.dev](https://alexisalbarenga.pages.dev)
- 💼 **LinkedIn:** [Alexis Albarenga](https://linkedin.com/in/alexis-albarenga-3b0324337)

---
<br clear="both">
<img src="https://raw.githubusercontent.com/Alexis217/Alexis217/output/snake.svg" alt="Snake animation" />

<p align="center">
  "Turning complex problems into elegant code."
</p>
